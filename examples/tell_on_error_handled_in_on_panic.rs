...
impl Actor for MyActor {
    ...
    async fn on_panic(
        ...
        if let Some(cont) = err.with_downcast_ref(|err: &MyError| {
            warn!("Actor would have panicked on tell error: {}", err);
...
impl Message<Request> for MyActor {
    ...
    async fn handle(&mut self, msg: Request, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
        Ok(msg.0)
    }
}
struct Fail(u32);
